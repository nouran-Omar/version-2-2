    ...
    {isHeartRate ? (
      <>
        <div className="flex justify-between items-start">
          <h2 className="text-[16px] font-bold tracking-tight">{label}</h2>
        ...
        <div className="flex items-end justify-between relative">
          <div className="z-10">
            ...
            <p className="text-[12px] mt-1">{status}</p>
          </div>
          <img src={'/Images/yes.svg'} alt="wave" className="absolute -top-8 -right-4 w-[65%] h-auto pointer-events-none" />
        </div>
      </>
    ) : (
      <>
        ...
        <div className="flex items-end justify-between relative">
          <div className="z-10">
            ...
            <div className="mt-2">
              <span
                className="text-xs font-normal font-roboto px-3 py-1 rounded-full inline-block"
          ...
          {chartSlot && (
            <div className="absolute -bottom-6 right-2 w-[60%] h-auto pointer-events-none overflow-visible">
              <img
                src={isLow ? "/Images/no.svg" : "/Images/yes.svg"}
                alt="wave"
                className="absolute -bottom-1 -right-11 w-full h-auto pointer-events-none"
              />
            </div>
          )}
        </div>
...
const PatientDashboard = () => {
  ...
  return (
    <section className="rounded-full flex flex-col font-roboto gap-6 p-[24px] md:p-[24px] bg-[#FAFBFF] min-h-screen">
      ...
      {/* Welcome Header */}
      <div className="mb-2">
        <h2 className="text-[24px] font-bold font-roboto text-black-main-text leading-tight">
          Welcome Back, {patient.name} 👋
        </h2>
        <p className="text-[18px] text-[#757575] font-roboto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          value={v.bloodPressure.display}
          unit="mmHg"
          status={v.bloodPressure.status}
          isLow
          icon={<TbDroplet />}
          chartSlot={<MiniChart path={BLOOD_PRESSURE_PATH} color="#F23985" gradId="bpGrad" />}
        />
        <StatCard
          label="Blood Sugar"
          ...
          chartSlot={<MiniChart path={BLOOD_SUGAR_PATH} color="#00DEA3" gradId="bsGrad" />}
        />
        <StatCard
          label="Blood Count"
          value={v.bloodCount?.value ?? 40}
          unit="g/dl"
          status={v.bloodCount?.status ?? 'Normal'}
          icon={<FaHeartPulse />}
          chartSlot={<MiniChart path={BLOOD_COUNT_PATH} color="#00DEA3" gradId="bcGrad" />}
        />
      </div>

      {/* Main Layout: Left (8) | Right (4) */}
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">

        {/* ===== Left Column (8 units) ===== */}
        <div className="xl:col-span-8 flex flex-col gap-6">

          {/* Weekly Chart — h-[600px] */}
          <div className="bg-white rounded-3xl p-4 shadow-sm border border-gray-50 overflow-hidden h-[600px]">
            <PatientWeeklyChart weeklyData={patient.weeklyData} />
          </div>

          {/* QR/AI Cards & Doctors List Row — h-[400px] */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-[400px]">
            ...
            <div className="lg:col-span-1 flex flex-col gap-3 h-full">
              ...
              <div
                ...
                className="flex flex-col gap-2 p-4 bg-slate-400 h-[97px] rounded-[24px] cursor-pointer hover:opacity-90 transition-all shadow-sm"
              >
              ...
              {/* AI Model Card */}
              <div className="flex flex-col gap-2  h-[97px] self-stretch px-4 py-3 bg-gradient-to-b from-[#5A8169] to-[#20E56F] rounded-[24px] shadow-sm relative overflow-hidden">
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
              ...
              <div className="space-y-2">
                <div className="w-full h-4 bg-[#75757566] rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-brand-main to-brand-dark"
                    style={{ width: '98%' }}
                  />
                </div>
                <div className="flex justify-between text-black-main-text items-center">
                  <span className="text-[12px] font-bold">⚙️ Accuracy : 98 %</span>
                </div>
              </div>
            ...
            {/* Right: Doctors List */}
            <div className="lg:col-span-2 bg-white p-6 rounded-[32px] h-[300px] border border-gray-100 shadow-sm flex flex-col h-full overflow-hidden">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-[15px] font-bold text-black-main-text tracking-tight">Doctors List</h3>
                <button
                  onClick={() => navigate('/patient/doctors')}
                  className="text-[11px] font-bold text-black-main-text hover:underline flex items-center gap-1"
                >
              ...
              <div className="flex flex-col gap-2 overflow-y-auto">
                {patient.featuredDoctors.slice(0, 3).map((doc) => (
                  <div
                    key={doc.id}
                    className="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-50 transition-all group border border-transparent hover:border-gray-50"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-bold text-black-main-text group-hover:text-blue-600 transition-colors">
                        {doc.name}
                      </p>
        ...
        <div className="xl:col-span-4 flex flex-col gap-6">
          ...
          {/* Health Summary Card */}
          <div className="bg-white rounded-[32px] w-full shadow-[0px_8px_24px_rgba(0,0,0,0.05)] border border-gray-100 p-5 md:p-6 flex items-center justify-between gap-4 transition-all hover:shadow-lg">
            <div className="flex flex-col gap-3 flex-1">
              <h3 className="text-[20px] md:text-[18px] font-bold text-black-main-text tracking-tight">
                Health Summary
              </h3>
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-[16px] md:text-[14px] font-semibold text-black-main-text">
                    Ai Risk Score
                  </span>
                  <span className="px-1.5 py-0.5 bg-[#E8F7EE] text-[#0C6D31] rounded-full text-[10px] tracking-wider border border-[#E8F7EE]">
                    {patient.aiRisk?.level || 'Low'} Risk
                  </span>
                </div>
                <p className="text-[14px] text-[#757575]">Your heart condition is stable.</p>
                <p className="text-[14px] text-[#757575]">Keep following your daily plan.</p>
              </div>
            </div>
            <div className="shrink-0 scale-90 md:scale-100">
              <HealthSummaryRing score={patient.aiRisk?.score || 95} color="#333CF5" />
            </div>
          </div>
            ...
            <div className="flex flex-col gap-4">
              {patient.appointments?.length > 0 ? (
                patient.appointments.slice(0, 2).map((apt) => (
                  <div
                    key={apt.id}
                    className="flex items-center justify-between p-3 rounded-2xl bg-[#F8FAFF] border border-blue-50/50 hover:bg-[#F0F4FF] transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <p className="text-black-main-text text-[13px] font-bold leading-tight">{apt.doctor}</p>
                        <p className="text-neutral-400 text-[11px] font-normal mt-0.5">{apt.location}</p>
                      </div>
                    </div>
                    <div className="text-right flex flex-col items-end gap-1">
                      ...
                      <p className="text-neutral-500 text-[10px] font-medium">{apt.date}</p>
                    </div>
                  </div>
                ))
              ) : (
